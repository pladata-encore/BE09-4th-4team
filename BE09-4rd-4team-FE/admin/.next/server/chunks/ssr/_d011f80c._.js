...
"[project]/src/components/NewUserModal.jsx [app-ssr] (ecmascript)": (function(__turbopack_context__) {

var { g: global, __dirname, m: module, e: exports } = __turbopack_context__;
{
const e = new Error(`Could not parse module '[project]/src/components/NewUserModal.jsx'
...
function UsersPage() {
    ...
    const handleAddUser = async (formData)=>{
        try {
            console.log("🧪 최종 API 요청 formData:", formData);
            const token = localStorage.getItem("accessToken"); // 로그인 후 저장된 토큰 불러오기
            if (!token) {
                console.error("❌ 토큰이 없습니다. 로그인 여부 확인 요망");
                alert("로그인이 필요합니다.");
                return;
            }
            const mappedForm = {
                ...formData,
                raw_password: formData.rawPassword
            };
            delete mappedForm.rawPassword;
            ...
            if (response.status === 201 || response.status === 200) {
                ...
                console.error('❌ 회원 등록 실패:', response.data);
                alert(`회원 등록 실패: ${response.data.message || '알 수 없는 오류'}`);
            }
        } catch (error) {
            if (error.response) {
                console.error("❌ 회원 등록 중 오류:", error.response.status, error.response.data);
                alert(`에러: ${error.response.data.message || '서버 오류 발생'}`);
            } else {
...
//# sourceMappingURL=_d011f80c._.js.map